...
export default function WhatsAppButton({ phoneNumber, storeName }: WhatsAppButtonProps) {
  ...
  const handleWhatsAppClick = () => {
    try {
      const message = `¡Hola ${storeName}! Vi tu catálogo y me interesa obtener más información.`;
      const encodedMessage = encodeURIComponent(message);
      const cleanPhone = formatPhoneNumber(phoneNumber);
      
      if (cleanPhone.length < 10) {
        alert('El número de teléfono no es válido. Por favor verifica el formato.');
        return;
      }
      ...
      alert('Error al abrir WhatsApp. Por favor verifica el número de teléfono.');
    }
  };
  ...
  return (
    <motion.button
      ...
      className="fixed bottom-6 right-6 bg-[#25D366] text-white p-3 rounded-full shadow-lg hover:bg-[#128C7E] transition-all duration-300 z-50 border-2 border-white"
